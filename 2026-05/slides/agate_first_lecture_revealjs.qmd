...
## Why another ecosystem modelling tool? {background-image="images/background-placeholder.svg" background-size="cover"}

::: {.slide-copy}
- Ecosystem model ideas often start as simple scientific changes: add a process, change a parameterization, split a type, or test a model variant.
- In practice, these changes are often implemented deep inside a specific host model.
- Once implemented, they may be hard to transfer to another model or coupling framework.
- Ecological model components should be easier to reuse.
:::

![](images/slide-01.svg){.slide-image}

## Scientific changes that do not transfer {background-image="images/background-placeholder.svg" background-size="cover"}

::: {.slide-copy}
- A new ecosystem process may depend on one tracer layout.
- A parameterization may be tied to one numerical grid or host model.
- Diagnostics and assumptions may be difficult to reproduce elsewhere.
- The same ecological idea may need to be reimplemented for every model.
:::
